...
import com.android.systemui.synth.FileImageUtils;
...
public class AmbientController implements ColorAnimator.ColorAnimationListener {
    ...
    private int mImageBlurRadius;
    private int mImageShowType;
    private int mImageHideType;
    ...
    private void updateVisibility() {
        ...
        boolean imageInQuickSettings = Settings.System.getIntForUser(mContext.getContentResolver(),
                Settings.System.AMBIENT_IMAGE_QS, 0, UserHandle.USER_CURRENT) != 0;
        boolean imageInAmbient = Settings.System.getIntForUser(mContext.getContentResolver(),
                Settings.System.AMBIENT_IMAGE_AOD, 1, UserHandle.USER_CURRENT) != 0;
        boolean imageInLockscreen = Settings.System.getIntForUser(mContext.getContentResolver(),
                Settings.System.AMBIENT_IMAGE_LOCKSCREEN, 0, UserHandle.USER_CURRENT) != 0;
        ...
        if (showInQuickSettings) {
            if (mImageState && imageInQuickSettings) showImage(); else hideImage();
            if (mTextState && textInQuickSettings) showText(); else hideText();
        } else if (showInAmbient) {
            if (mImageState && imageInAmbient) showImage(); else hideImage();
            if (mTextState && textInAmbient) showText(); else hideText();
            if (mImageState || mTextState) {
            ...
            if (mImageState && imageInLockscreen) showImage(); else hideImage();
    ...
    private void showImage() {
        ...
        mCustomImageView.setTranslationX(0);

        ValueAnimator animationImageAlpha = ValueAnimator.ofInt(0, 255);
        animationImageAlpha.setDuration(ANIMATION_DURATION);
        animationImageAlpha.setInterpolator(new FastOutSlowInInterpolator());
        animationImageAlpha.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            public void onAnimationUpdate(ValueAnimator animation) {
                mCustomImageView.setAlpha((int) animation.getAnimatedValue());
                mCustomImageView.setImageAlpha((int) animation.getAnimatedValue());
                mCustomImageView.invalidate();
            }
        });

        switch (mImageShowType) {
            case 0:
                transition = new TransitionSet()
                    .addTarget(mCustomImageView)
        ...
        TransitionManager.beginDelayedTransition((ViewGroup) mContainer, transition);
        // mContainer.setVisibility(View.VISIBLE);
        mCustomImageView.setVisibility(View.VISIBLE);
        animationImageAlpha.start();
        mVisibility = true;
    }
    ...
    private void hideImage() {
        ...
        ValueAnimator animationImageAlpha = ValueAnimator.ofInt(255, 0);
        animationImageAlpha.setDuration(ANIMATION_DURATION);
        animationImageAlpha.setInterpolator(new FastOutSlowInInterpolator());
        animationImageAlpha.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            public void onAnimationUpdate(ValueAnimator animation) {
    ...
    private void updateImage() {
        ...
        if (imageUri != null) {
            if (mImageBlur) {
                Bitmap bitmapBlur = Dependency.get(NotificationMediaManager.class).getMediaArtworkProcessor().processArtwork(mContext, bitmap, mImageBlurRadius);
                mCustomImageView.setImageBitmap(bitmapBlur);
                mCustomImageView.invalidateOutline();
            } else {
    ...
    private void updateColor() {
        ...
        switch (mTextColorType) {
            ...
            case 4: // Gradient 
                ...
                break;
            case 5: // Background
                String customNameImage = "AmbientTextBackground";
                String imageUri = Settings.System.getString(mContext.getContentResolver(),
                    Settings.System.AMBIENT_TEXT_GRAVITY);
                Bitmap bitmap = null;
                if (imageUri != null) {
                    FileImageUtils.saveImage(mContext, Uri.parse(imageUri), customNameImage);
                    bitmap = FileImageUtils.loadImage(mContext, customNameImage);
                }
                if (bitmap != null) {
                    BitmapShader bitmapShader = new BitmapShader(bitmap, Shader.TileMode.REPEAT, Shader.TileMode.REPEAT);
                    mTextView.getPaint().setShader(bitmapShader);
                } else {
                    mTextView.getPaint().setShader(null);
                    mTextView.setTextColor(getAccentColor());
                }
    ...
    private class SettingsObserver extends ContentObserver {
        ...
        void register() {
            mContext.getContentResolver().registerContentObserver(
                    Settings.System.getUriFor(Settings.System.AMBIENT_IMAGE_ENABLED), false, this,
                    UserHandle.USER_ALL);
            mContext.getContentResolver().registerContentObserver(
                    Settings.System.getUriFor(Settings.System.AMBIENT_CUSTOM_IMAGE), false, this,
                    UserHandle.USER_ALL);
            mContext.getContentResolver().registerContentObserver(
                    Settings.System.getUriFor(Settings.System.AMBIENT_IMAGE_BLUR), false, this,
                    UserHandle.USER_ALL);
            mContext.getContentResolver().registerContentObserver(
                    Settings.System.getUriFor(Settings.System.AMBIENT_IMAGE_BLUR_RADIUS), false, this,
                    UserHandle.USER_ALL);
            mContext.getContentResolver().registerContentObserver(
                    Settings.System.getUriFor(Settings.System.AMBIENT_IMAGE_SHOW_TYPE), false, this,
                    UserHandle.USER_ALL);
            mContext.getContentResolver().registerContentObserver(
                    Settings.System.getUriFor(Settings.System.AMBIENT_IMAGE_HIDE_TYPE), false, this,
                    UserHandle.USER_ALL);
            mContext.getContentResolver().registerContentObserver(
        ...
        public void onChange(boolean selfChange, Uri uri) {
            if (uri.equals(Settings.System.getUriFor(Settings.System.AMBIENT_CUSTOM_IMAGE))) {
                updateImage();
            } else if (uri.equals(Settings.System.getUriFor(Settings.System.AMBIENT_IMAGE_ENABLED)) ||
                        uri.equals(Settings.System.getUriFor(Settings.System.AMBIENT_IMAGE_BLUR)) ||
                        uri.equals(Settings.System.getUriFor(Settings.System.AMBIENT_IMAGE_BLUR_RADIUS))) {
                updateEnabled();
            } else if (uri.equals(Settings.System.getUriFor(Settings.System.AMBIENT_TEXT_GRAVITY)) ||
        ...
        void updateEnabled() {
            mImageState = Settings.System.getInt(mContext.getContentResolver(),
                    Settings.System.AMBIENT_IMAGE_ENABLED, 0) == 1;
            mImageBlur = Settings.System.getIntForUser(mContext.getContentResolver(),
                    Settings.System.AMBIENT_IMAGE_BLUR, 0, UserHandle.USER_CURRENT) == 1;
            mImageBlurRadius = Settings.System.getIntForUser(mContext.getContentResolver(),
                    Settings.System.AMBIENT_IMAGE_BLUR_RADIUS, 5, UserHandle.USER_CURRENT);
            mImageShowType = Settings.System.getIntForUser(mContext.getContentResolver(),
                    Settings.System.AMBIENT_IMAGE_SHOW_TYPE, 0, UserHandle.USER_CURRENT);
            mImageHideType = Settings.System.getIntForUser(mContext.getContentResolver(),
                    Settings.System.AMBIENT_IMAGE_HIDE_TYPE, 0, UserHandle.USER_CURRENT);
        }
